...
export async function main(ns: NS) {
    ...
    while (true) {
        ...
        const filteredServers = servers
            .map((s) => ({ hostname: s, route: serverPath(ns, s), server: ns.getServer(s) }))
            .filter(
                ({ server }) =>
                            ...
                            return (
                                <tr key={hostname}>
                                    ...
                                    <td style={{ color: getColorScale(server.moneyAvailable / server.moneyMax) }}>
                                        {server.moneyMax === 0
                                            ? ""
                                            : Math.round((server.moneyAvailable / server.moneyMax) * 1000) / 10 + "%"}
                                    </td>
                                    <td>{ns.nFormat(server.moneyMax, "$0.00a")}</td>
                                    <td
                                        style={{
                                            color: getColorScale(
                                                1 - (server.hackDifficulty - server.minDifficulty) / 10
                                            )
                                        }}>
