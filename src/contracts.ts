        ...
        for (j = i + 1; j < arrayData.length - 2; j++) {
            for (k = j + 1; k < arrayData.length - 1; k++) {
                ...
                for (l = 0; l < 4; l++) {
                    ...
                    if (tempOctet.slice(0, 1) === "0") {
                        isValid = 0
                    }
                    if (parseInt(tempOctet) > 255) {
                        isValid = 0
                    }
                }
                if (isValid) {
...
async function solverStockTrader(ns: NS, arrayData: any) {
    ...
    for (i = 0; i < arrayData[0]; i++) {
        for (j = 0; j < arrayData[1].length; j++) {
            // Buy / Start
            for (k = j; k < arrayData[1].length; k++) {
                // Sell / End
                if (i > 0 && j > 0 && k > 0) {
                    highestProfit[i][k] = Math.max(
                        ...
                        highestProfit[i - 1][k],
                        highestProfit[i][k - 1],
                        highestProfit[i - 1][j - 1] + arrayData[1][k] - arrayData[1][j]
                    )
                } else if (i > 0 && j > 0) {
                    highestProfit[i][k] = Math.max(
                        highestProfit[i][k],
...
async function solverTrianglePath(ns: NS, arrayData: any) {
    ...
    for (i = 1; i < arrayData.length; i++) {
        for (j = 0; j < arrayData[i].length; j++) {
            arrayData[i][j] += Math.min(
                arrayData[i - 1][Math.max(0, j - 1)],
                arrayData[i - 1][Math.min(j, arrayData[i - 1].length - 1)]
            )
        }
    }
        ...
        for (j = 1; j < arrayData[0].length; j++) {
            ...
            switch (rollingOperator) {
                case "":
                    rollingOperand =
                        rollingOperand *
                        (arraySummands[arraySummands.length - 1] / Math.abs(arraySummands[arraySummands.length - 1]))
                    arraySummands[arraySummands.length - 1] =
                        arraySummands[arraySummands.length - 1] * 10 + rollingOperand
                    break
                case "+":
...
async function solverWaysToSum(ns: NS, arrayData: any) {
    let precalcPartitions = [
        ...
        53173, 63260, 75174, 89133, 105557, 124753, 147272, 173524, 204225, 239942, 281588, 329930, 386154, 451275,
        526822, 614153, 715219, 831819, 966466, 1121504, 1300155, 1505498, 1741629, 2012557, 2323519, 2679688, 3087734,
        3554344, 4087967, 4697204, 5392782, 6185688, 7089499, 8118263, 9289090, 10619862, 12132163, 13848649, 15796475,
        18004326, 20506254, 23338468, 26543659, 30167356, 34262961, 38887672, 44108108, 49995924, 56634172, 64112358,
        72533806, 82010176, 92669719, 104651418, 118114303, 133230929, 150198135, 169229874, 190569291, 214481125,
        241265378, 271248949, 304801364, 342325708, 384276335, 431149388, 483502843, 541946239, 607163745, 679903202,
        761002155, 851376627, 952050664, 1064144450, 1188908247, 1327710075, 1482074142, 1653668664, 1844349559,
        2056148050, 2291320911, 2552338240, 2841940499, 3163127351, 3519222691, 3913864294, 4351078599, 4835271869,
        5371315399, 5964539503, 6620830888, 7346629511, 8149040694, 9035836075, 10015581679, 11097645015, 12292341830,
        13610949894, 15065878134, 16670689207, 18440293319, 20390982756, 22540654444, 24908858008, 27517052598,
        30388671977, 33549419496, 37027355199
    ]
